...
// export default function Jury2024() {
//   const juryMembers = [
//     {
//       id: 1,
//       name: 'Jury Member 1',
...
//         <h1 className="jury-title">JURY 2024</h1>
...
export default function Jury2024() {
  ...
  const [pageTitle, setPageTitle] = useState('JURY 2024');
  ...
  useEffect(() => {
    async function loadJuryData() {
      try {
        ...
        const response = await fetchWebsitePageBySlug('jury-2024');

        if (!response?.success || !response?.data) {
          throw new Error('Unable to fetch jury data');
        }

        setPageTitle(response.data.title || 'JURY 2024');
      ...
      <section className="jury-hero">
        <div className="jury-container">
          ...
          <p className="jury-subtitle">
            Meet the distinguished jury members evaluating and recognizing exceptional leadership,
